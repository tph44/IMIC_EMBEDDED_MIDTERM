...
//  for (int i = 0; i < 7; i++) {
//	  read_data_buffer = *(uint32_t*)(flash_addr + read_cnt);
//	  data[read_cnt] 		= read_data_buffer;
//	  read_cnt += 4;
//  }
...
//new_fa_status = check_info(fw_msg);
  ...
  // uart_send_string("Da nhan du firmware, tien hanh update\n");

  // Flash_Erase(0);
  // Flash_Program(0x08000000, new_fw, sizeof(new_fw));
